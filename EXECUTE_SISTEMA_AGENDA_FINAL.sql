...
-- SCRIPT FINAL PARA SUPABASE - SEM CONFLITOS
-- ========================================
...
-- 5. REMOVER VIEWS EXISTENTES E RECRIAR
...
-- 6. RECRIAR VIEW AGENDAMENTOS COMPLETO COM NOVOS CAMPOS
...
-- 8. VIEW PARA SUPERVISÃO (ATENDIMENTOS POR SALA E PROFISSIONAL)
...
SELECT 
    ...
    -- Sessões em andamento com tempo (calculado aqui)
    STRING_AGG(
        CASE WHEN a.status = 'em_atendimento' THEN 
            p.nome || ' (' || 
            CASE 
                WHEN a.sessao_iniciada_em IS NOT NULL THEN
                    EXTRACT(EPOCH FROM (NOW() - a.sessao_iniciada_em)) / 60
                ELSE a.tempo_sessao_minutos
            END::INTEGER || 'min)'
        END, 
...
SELECT 
    ...
    '✅ Views removidas e recriadas' as step_4,

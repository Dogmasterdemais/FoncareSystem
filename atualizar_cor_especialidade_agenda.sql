...
SELECT 
    ...
    -- Status e controle
    COALESCE(a.status_atendimento, 'agendado') as status,
    
    -- Status dinâmico baseado em lógicas
    CASE 
        WHEN a.status_atendimento = 'em_atendimento' AND 
             EXTRACT(EPOCH FROM (NOW() - a.created_at))/60 >= a.duracao_minutos 
        THEN 'sessao_completa'
        
        WHEN a.status_atendimento = 'em_atendimento' 
        THEN 'em_andamento'
        
        ELSE COALESCE(a.status_atendimento, 'agendado')
    ...
    c1.nome as profissional_1_nome,
    c1.nome as profissional_2_nome,  -- Mesmo profissional por enquanto
    1 as profissional_ativo,
    
    -- Próxima ação sugerida
    CASE 
        WHEN a.status_atendimento = 'agendado' THEN 'Aguardando chegada do paciente'
        WHEN a.status_atendimento = 'chegou' THEN 'Realizar tabulação da guia'
        WHEN a.status_atendimento = 'pronto_para_terapia' THEN 'Iniciar atendimento'
        WHEN a.status_atendimento = 'em_atendimento' AND 
             EXTRACT(EPOCH FROM (NOW() - a.created_at))/60 >= a.duracao_minutos 
        THEN 'Finalizar atendimento'
        WHEN a.status_atendimento = 'em_atendimento' THEN 'Atendimento em andamento'
        WHEN a.status_atendimento = 'concluido' THEN 'Atendimento finalizado'
    ...
    a.updated_at as hora_fim_atendimento,
...
ON agendamentos(status_atendimento) 
WHERE ativo = true;

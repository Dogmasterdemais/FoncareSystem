...
-- SCRIPT PARA SUPABASE - COLE ESTE SCRIPT COMPLETO
-- ========================================
...
CREATE OR REPLACE VIEW vw_supervisao_atendimentos AS
SELECT 
    u.nome as unidade_nome,
    s.nome as sala_nome,
    s.numero as sala_numero,
...
-- 9. POLICIES RLS (SECURITY)
ALTER TABLE agendamentos_status_log ENABLE ROW LEVEL SECURITY;

-- Remover políticas existentes se houver
DROP POLICY IF EXISTS "Usuarios podem ver logs de agendamentos de sua unidade" ON agendamentos_status_log;
DROP POLICY IF EXISTS "Usuarios podem inserir logs de agendamentos" ON agendamentos_status_log;

-- Criar políticas
CREATE POLICY "Usuarios podem ver logs de agendamentos de sua unidade" ON agendamentos_status_log
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM agendamentos a 
            WHERE a.id = agendamento_id 
            AND a.unidade_id = (auth.jwt() ->> 'unidade_id')::uuid
        )
    );

CREATE POLICY "Usuarios podem inserir logs de agendamentos" ON agendamentos_status_log
...
SELECT 
    ...
    '✅ Políticas de segurança aplicadas' as step_6;

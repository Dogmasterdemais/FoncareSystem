-- ============================================================================
-- SISTEMA AUTOMATIZADO DE TRANSIÇÕES DE PROFISSIONAIS - VERSÃO SALAS
-- ============================================================================
-- Este arquivo implementa transições automáticas sem botões manuais
-- O sistema detecta automaticamente quando trocar de profissional baseado no tempo
...
'Processa transições automáticas de profissionais baseado no tempo decorrido. 
Executa automaticamente as trocas:
- Tripla: Prof1(0-30min) → Prof2(30-60min) → Prof3(60-90min)
- Compartilhada: Prof1(0-30min) → Prof2(30-60min)  
- Individual: Prof1(0-60min)';

-- Criar view atualizada com detecção automática de status
...
-- Comentário da view
COMMENT ON VIEW vw_agenda_tempo_real_automatica IS 
'View para agenda em tempo real com transições automáticas de profissionais.
Status dinâmico e profissional ativo são calculados automaticamente baseado no tempo decorrido.
Não requer intervenção manual - o sistema gerencia as transições automaticamente.';

-- Função para executar processamento automático (pode ser chamada por cron/scheduler)
...
'Função principal para executar o processamento automático de transições.
Pode ser chamada por um scheduler/cron a cada minuto para manter as transições automáticas.';

-- Logs para debug
SELECT 'Sistema automatizado de transições implementado com sucesso!' as status;
SELECT 'Use: SELECT executar_processamento_automatico(); para processar transições' as instrucao;
SELECT 'Use: SELECT * FROM vw_agenda_tempo_real_automatica; para ver agenda atualizada' as consulta;

      ...
      const { data, error } = await supabase
        ...
        .select(`
          *,
          ...
          convenios(nome)
        `)
        .eq(unidadeId ? 'unidade_id' : '1', unidadeId || '1')
        .order('data_vencimento', { ascending: true });
      
      if (error) throw error;
      
      // Mapear dados para incluir nomes
      const contasFormatadas = (data || []).map(conta => ({
        ...conta,
        paciente_nome: conta.pacientes?.nome || 'N/A',
        convenio_nome: conta.convenios?.nome || conta.origem
      }));
      
      setContas(contasFormatadas);
    } catch (error) {
      console.error('Erro ao carregar contas a receber:', error);
      setContas([]); // Definir array vazio em caso de erro
    } finally {
      setLoading(false);
    }
  }
